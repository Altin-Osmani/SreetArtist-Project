const cardsWrapper = document.querySelector(".cards-wrapper") // inserted cards div
const noItemsOverlay = document.querySelector(".no-items-screen-overlay")
const noItemsPopup = document.querySelector(".no-items-popup") //shfaqja e errorit
const backToFiltersBtn = document.querySelector(".back-to-filters") //butoni i errorit
...
// Filter sliding
// *********************
const listingPage = document.getElementById("visitor/listing") //SEKSIONI
const filterBtn = document.querySelector(".filter-icon") //
const filtersContainer = document.querySelector(".filters") // divi i INPUTEve PER SELEKTIMIN E IMAZHIT
const closeBtn = document.querySelector(".close-btn") //BUTONI X
...
//Marja e values nga LS
const getItem = (key) =>{ // Gets values from local storage
	let item = JSON.parse(localStorage.getItem(key)) //key (jan valuet)
	return item
}
...
const filterCards = () =>{
	...
	if(filteredItems.length === 0){
		noItemsOverlay.classList.add("active") //divi i errorit
